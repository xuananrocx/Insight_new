...
def outline(kb,file,offset=0,query='',object_id=None):
    ...
    with reading_index.open_index(file) as conn:
        ...
        rows = conn.execute('SELECT id,label,length(text) AS chars FROM sections ORDER BY id LIMIT 51 OFFSET ?',(offset,)).fetchall()
        ...
        result = {**kb.brief(file),'index_status':'ready','index_version':reading_index.VERSION,
                  'sections':[dict(section=r['id'],label=r['label'],chars=r['chars'],read_ref=reference(kb,file,info['artifact'],r['id'])) for r in rows[:50]],
