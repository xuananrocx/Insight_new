...
function UserDetails({ target, roles, onClose }: { target: Account; roles: Role[]; onClose: () => void }) {
  ...
  return <ManagementDialog title={`${target.username} · 账号详情`} busy={busy} onClose={onClose}><div className="flex flex-wrap gap-2">{[['profile', '基本资料'], ['roles', '角色权限'], ['resources', '资源权限'], ...(can('users.sessions') && manageable ? [['sessions', '登录会话']] : [])].map(([value, label]) => <Button key={value} size="sm" variant={tab === value ? 'default' : 'outline'} onClick={() => { setTab(value); setPreview(null) }}>{label}</Button>)}</div>
    ...
    {tab === 'sessions' && <div className="space-y-3"><p className="text-sm">当前有效登录：{sessions.data?.length ?? '…'} 个</p>{sessions.error && <p className="text-destructive">{sessions.error.message}</p>}{sessions.data?.map((s, i) => <p className="text-xs text-muted-foreground" key={i}>到期时间：{new Date(s.expires_at * 1000).toLocaleString()}</p>)}{other && <Button variant="outline" disabled={busy} onClick={async () => { if (!window.confirm(`让 ${target.username} 的全部登录会话退出？`)) return; setBusy(true); try { await accountRequest(`/admin/users/${target.id}/sessions`, 'DELETE'); await sessions.refetch(); toast.success('登录会话已撤销') } catch (e) { error(e) } finally { setBusy(false) } }}>强制全部退出</Button>}</div>}
...
export function RolesPage() {
  ...
  return <Page title="角色管理" description="自定义角色并为账号分配多个角色。功能权限与具体知识库、API 的授权分开设置；AI 增强和深度 AI 不设角色门槛。">
